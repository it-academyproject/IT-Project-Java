...
public class EmailService {
		...
		for (int i = 0; i < absence.size(); i++) {
			...
			if (counter == 8) {
				...
				for (int j = 0; j < emailsSent.size(); j++) {
					if (emailsSent.get(j).isSent() == true) {
	...
	///////////////////////////////////// 8 DAYS WITHOUT DELIVERY EXECISE PER STUDENT///////////////

	public void notificationEmailExceedDeliveryTime() {
	...
	public void emailNotification(String email, String name, String messageBody) throws Exception {
		...
		helper.setTo(email);
		helper.addBcc("barcelonaactiva.itacademy@gmail.com"); ////// WRITE THE EMAIL FROM IT ACADEMY WHO WILL RECEIVE
																////// THE NOTIFICATIONS//////
