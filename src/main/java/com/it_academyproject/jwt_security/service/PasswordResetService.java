...
public class PasswordResetService
{
	...
	public JSONObject resetPassword(String email) throws UserNotFoundException, Exception
	{
		...
		if (myAppUser != null && myAppUser.isEnabled())
		{
			...
			List<PasswordResetToken> passwordResetTokenList = passwordResetTokenRepository
					.findByMyAppUserId(myAppUser.getId());
			...
			try
			{
				...
				String email_content = "<h2>Hello User!</h2>"
						...
						+ "				<p> If correct, please follow this link to do so: </p> " + email_link;
	...
	public JSONObject savePassword(String body)
			throws InvalidFormatException, InvalidToken, EmptyFieldException, Exception
	{
		...
		if (getData.has("token") && getData.has("email") && getData.has("password") && getData.has("confirm_password")
				&& !getData.getString("token").equals("") && !getData.getString("email").equals("")
				&& !getData.getString("password").equals("") && !getData.getString("confirm_password").equals("")
